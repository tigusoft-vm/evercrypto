...
string generate_different_string (const string &msg) {
	...
	switch (random) {
		...
		case 6:
			...
			break;
		case 7:
			ret = ret.substr(0, ret.size() / 2) + generate_random_string(50) +
				  ret.substr(ret.size() / 2, ret.size() / 2);
			break;
		case 8:
...
bool check_equality_of_signature (const c_crypto_geport_def::signature_t &a,
		const c_crypto_geport_def::signature_t &b) {
...
c_crypto_geport_def::signature_t generate_different_signature (const c_crypto_geport_def::signature_t &signature) {
	...
	switch (random) {
		case 0:
			ret.Signature[get_rand() % (c_crypto_geport_def::signature_or_private_key_length)] -=
					(get_rand() % 10000000L) + 1;
			break;
		case 1:
...
c_crypto_geport_def::public_key_t generate_different_public_key (c_crypto_geport_def::public_key_t &public_key) {
	...
	if (ret == public_key)
		return generate_different_public_key(public_key);
	else
		return ret;
}
...
void print_signed_msg (const c_crypto_geport_def::signature_t &signature,
		const c_crypto_geport_def::public_key_t &public_key) {
