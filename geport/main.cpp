...
void show_help () {
  ...
  cout << "-v\t--verify-public-key [priv_key pub_key]\t\tcheck whether given public key is corresponding with given private key" << endl;
...
void check_signature (const string &filename, const string &signature_filename, const string &pub_key_filename) {
  ...
  if (is_ok) cout << "signature is OK\n";
  else cout << "signature is WRONG\n";
}
...
int main (int argc, char *argv[]) {
  ...
  if (command == "--help") {
    ...
    string file(argv[2]);
    cout << sha512<number<cpp_int_backend<512 * 2, 512 * 2, unsigned_magnitude, unchecked, void>>>(file_to_string(file)) << "  " << file << '\n';
    ...
    string file(argv[2]);
    cout << sha256<number<cpp_int_backend<256 * 2, 256* 2, unsigned_magnitude, unchecked, void>>>(file_to_string(file)) << "  " << file << '\n';
